...
interface GroupingResult {
  teamA: GroupMember[];
  teamB: GroupMember[];
  skillSummary: SkillDistribution;
}
 ...
 */
function shuffle(array: GroupMember[]): GroupMember[] {
  const arr = [...array];
 ...
 */
function balancedGroupBySkillLevel(members: GroupMember[]): GroupingResult {
  // 1. 按档位分组
  const skillGroups: Record<SkillLevel, GroupMember[]> = {
    5: [], // 核心球员
    4: [], // 主力球员
    3: [], // 常规球员
    2: []  // 新手球员
  ...
  members.forEach(member => {
    const level: SkillLevel = member.skillLevel || 3; // 未设置默认为3档
  ...
  for (let level = 5; level >= 2; level--) {
    const count = skillGroups[level as SkillLevel].length;
    if (count % 2 !== 0) {
      throw new Error(
  ...
  // 3. 初始化两队
  const teamA: GroupMember[] = [];
  const teamB: GroupMember[] = [];

  // 4. 对每个档位进行随机均分（从高到低）
  for (let level = 5; level >= 2; level--) {
    const group = shuffle(skillGroups[level as SkillLevel]); // 随机洗牌
  ...
  const sortBySkillAndNumber = (a: GroupMember, b: GroupMember): number => {
    // 先按档位降序（5档在前）
    if (b.skillLevel !== a.skillLevel) {
      return (b.skillLevel || 3) - (a.skillLevel || 3);
    }
    // 同档位按号码升序
  ...
  // 6. 生成档位统计
  const skillSummary: SkillDistribution = {
 ...
 */
function generateSkillDistribution(members: GroupMember[]): SkillDistribution {
  return {
...
// 示例球员数据
const examplePlayers: GroupMember[] = [
 ...
 */
function validateGroupingRequirements(members: GroupMember[]): ValidationResult {
  const stats: SkillDistribution = {
    level5: 0,
    level4: 0,
    level3: 0,
    level2: 0,
  ...
  // 统计各档位人数
  members.forEach(member => {
    const level: SkillLevel = member.skillLevel || 3;
    stats[`level${level}` as keyof SkillDistribution] += 1;
  });
  ...
  const errors: string[] = [];
  for (let level = 5; level >= 2; level--) {
    const count = stats[`level${level}` as keyof SkillDistribution];
