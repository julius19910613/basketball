...
type LevelGroups = Record<PlayerLevel, GroupPlayer[]>;
...
interface BalancedGroupOptions {
...
interface GroupConditionDistribution {
  level: number;
  count: number;
}

interface GroupConditionResult {
  valid: boolean;
  errors: string[];
  warnings: string[];
  levelDistribution: GroupConditionDistribution[];
}
 ...
 */
function groupByLevel(players: GroupPlayer[]): LevelGroups {
  ...
  players.forEach((player: GroupPlayer) => {
    const level = player.skillLevel as PlayerLevel;
    if (groups[level]) {
 ...
 * @returns {Object} 分组结果
 */
function balancedGroup(players: GroupPlayer[], teamCount = 2, options: BalancedGroupOptions = {}): BalancedGroupResult {
  ...
  Object.keys(levelGroups).forEach((level) => {
    const groupLevel = Number(level) as PlayerLevel;
    levelGroups[groupLevel] = shuffle(levelGroups[groupLevel]);
  });
  ...
  // 从高档位到低档位依次分配（蛇形分配）
  const levels: PlayerLevel[] = [5, 4, 3, 2];

  levels.forEach((level: PlayerLevel) => {
    ...
    while (playerIndex < levelPlayers.length) {
      ...
      teams.forEach((team: GroupTeam, idx: number) => {
  ...
  // 按名称排序（A、B、C...）
  teams.sort((a: GroupTeam, b: GroupTeam) => a.name.localeCompare(b.name));
 ...
 * @returns {Object} 统计信息
 */
function calculateGroupStats(teams: GroupTeam[]): GroupStats | null {
  if (teams.length === 0) return null;

  const avgLevels = teams.map((t: GroupTeam) => parseFloat(String(t.avgLevel)));
  const maxAvg = Math.max(...avgLevels);
  const minAvg = Math.min(...avgLevels);
  const totalPlayers = teams.reduce((sum: number, t: GroupTeam) => sum + t.players.length, 0);
 ...
 * @returns {Object} 验证结果
 */
function validateGroupCondition(selectedPlayers: GroupPlayer[], teamCount: number): GroupConditionResult {
  const errors: string[] = [];
  ...
  const levelCounts = Object.keys(levelGroups).map((level) => ({
    ...
    count: levelGroups[Number(level) as PlayerLevel].length
  }));

  // 警告：某个档位人数过少可能导致不平衡
  levelCounts.forEach((item: GroupConditionDistribution) => {
 ...
 */
function getGroupPresets(playerCount: number): GroupPreset[] {
